...
def test_build_scoring_matrx():
    chunks = [TextChunk(id=elem['ID'], text=elem['CHUNK']) for elem in json.loads(open('sample_summary.json', encoding='utf-8').read())]
    turns = [TextChunk(id=elem['ID'], text=elem['TURN']) for elem in json.loads(open('sample_turns.json', encoding='utf-8').read())]
    ...
    s = NeedlemanWunschAligner.build_scoring_matrx(tokenized_turns, tokenized_chunks, calculate_scaled_f1_turn_distribution)
